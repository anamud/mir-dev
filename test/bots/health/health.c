...
{ /*{{{*/
   ...
   else
   {
      ...
      *capital = (struct Village *) malloc(sizeof(struct Village));
      ...
      mir_lock_create(&(*capital)->hosp.realloc_lock);
      ...
      { 
         ...
         if (rand < sim_convalescence_p)
         {
            ...
            else /* move to upper level hospital !!! */
            {
               village->hosp.free_personnel++;
               removeList(&(village->hosp.assess), p);
               mir_lock_set(&(village->hosp.realloc_lock));
               addList(&(village->back->hosp.realloc), p); 
               mir_lock_unset(&(village->hosp.realloc_lock));
            } 
         }
      ...
      {
        {
            ...
            struct mir_task_t* task = mir_task_create((mir_tfunc_t) smp_ol_sim_village_par_0, (void*) &imm_args, sizeof(imm_args), twc, 0, NULL, NULL); 
        }
...
{/*{{{*/
   ...
   if ((fin = fopen(filename, "r")) == NULL) {
      PMSG("Could not open sequence file (%s)\n", filename);
      exit (-1);
   }
   ...
   fclose(fin);
      ...
      // Printing input data
   PMSG("\n");
   PMSG("Number of levels    = %d\n", (int) sim_level);
   PMSG("Cities per level    = %d\n", (int) sim_cities);
   PMSG("Population ratio    = %d\n", (int) sim_population_ratio);
   PMSG("Simulation time     = %d\n", (int) sim_time);
   PMSG("Assess time         = %d\n", (int) sim_assess_time);
   PMSG("Convalescence time  = %d\n", (int) sim_convalescence_time);
   PMSG("Initial seed        = %d\n", (int) sim_seed);
   PMSG("Get sick prob.      = %f\n", (float) sim_get_sick_p);
   PMSG("Convalescence prob. = %f\n", (float) sim_convalescence_p);
   PMSG("Realloc prob.       = %f\n", (float) sim_realloc_p);
}/*}}}*/
...
{/*{{{*/
   ...
   if (res_population != result.total_patients) answer = TEST_UNSUCCESSFUL;
   if (res_hospitals != result.hosps_number) answer = TEST_UNSUCCESSFUL;
   if (res_personnel != result.hosps_personnel) answer = TEST_UNSUCCESSFUL;
   if (res_checkin != result.total_hosps_v) answer = TEST_UNSUCCESSFUL;
   if (res_village != result.total_in_village) answer = TEST_UNSUCCESSFUL;
   if (res_waiting != result.total_waiting) answer = TEST_UNSUCCESSFUL;
   if (res_assess != result.total_assess) answer = TEST_UNSUCCESSFUL;
   if (res_inside != result.total_inside) answer = TEST_UNSUCCESSFUL;

   PMSG("\n");
   PMSG("Sim. Variables      = expect / result\n");
   PMSG("Total population    = %6d / %6d people\n", (int)   res_population, (int) result.total_patients);
   PMSG("Hospitals           = %6d / %6d people\n", (int)   res_hospitals, (int) result.hosps_number);
   PMSG("Personnel           = %6d / %6d people\n", (int)   res_personnel, (int) result.hosps_personnel);
   PMSG("Check-in's          = %6d / %6d people\n", (int)   res_checkin, (int) result.total_hosps_v);
   PMSG("In Villages         = %6d / %6d people\n", (int)   res_village, (int) result.total_in_village);
   PMSG("In Waiting List     = %6d / %6d people\n", (int)   res_waiting, (int) result.total_waiting);
   PMSG("In Assess           = %6d / %6d people\n", (int)   res_assess, (int) result.total_assess);
   PMSG("Inside Hospital     = %6d / %6d people\n", (int)   res_inside, (int) result.total_inside);
   PMSG("Average Stay        = %6f / %6f u/time\n", (float) res_avg_stay,(float) result.total_time/result.total_patients);
