...
Enable the -r flag to get detailed per-thread state and event information in files with *-prv-*.rec extension.
...
A set of files matching the pattern *-state-time*.rec are also created when -r is enabled. 
...
Next enable PAPI hardware performance counters of interest in MIR\_ROOT/src/mir\_recorder.c.
Rebuild MIR.
