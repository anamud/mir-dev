...
Threads are released when mir\_destoy is called.
...
\item Do not use the parallel contruct to share work. 
\item Do not use barriers to synchronize threads.
\item Use the task contruct to parallelize work. Use clauses shared, firstprivate and private to indicate the data environment.
\item Use taskwait to synchronize tasks.
\item A simple technique which I use often is as follows: Create the runtime system right in the begining of the the program and release it at the end of the program. When parallel execution is required, create a \texttt{parallel} block followed immediately by a \texttt{single} block. Use the \texttt{task} construct within the \texttt{single} block to parallelize work. Synchronize the tasks using the \texttt{taskwait} construct. Before compiling, comment out the parallel and single blocks.
\item Use mir\_lock instead of the \texttt{critical} construct. 
\item Use gcc atomic builtins for flusing and atomic operations.
