...
Parallel for-loops are executed as special tasks by MIR. To enable per-chunk statistics, set \texttt{--chunks-are-tasks} in \texttt{MIR\_CONF}. Process for-loop metrics using special scripts in the \textsf{for-loop} directory in \textsf{scripts/profiling/task}. These scripts have the same name as scripts that process task metrics, except for the prefix \textsf{loop-}.
