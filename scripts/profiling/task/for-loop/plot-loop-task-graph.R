...
if (Rstudio_mode) {
    parsed <- list(data="loop-task-stats.processed",
                   ...
                   cplengthonly=F,
                   analyze=T,
                   config="loop-task-graph-analysis.cfg",
                   verbose=T,
                   timing=F)
} else {
    option_list <- list(
                        ...
                        make_option(c("-o","--out"), default="loop-task-graph", help = "Output file suffix [default \"%default\"].", metavar="STRING"),
                        ...
                        make_option(c("--config"), default="loop-task-graph-analysis.cfg", help = "Analysis configuration file [default \"%default\"].", metavar="FILE"),
...
attrib_color_scaled <- c("mem_fp", "-compute_int", "PAPI_RES_STL_sum", "-mem_hier_util", "work_deviation", "overhead_deviation", "-parallel_benefit", "-min_shape_contrib", "-max_shape_contrib","-median_shape_contrib", "sibling_work_balance", "chunk_work_balance", "chunk_work_cpu_balance")
...
my_print("Degree distribution:")
degree.distribution(tg)
my_print()
sink()
