...
{ /*{{{*/
    ...
    if (runtime != NULL) {
        MIR_ASSERT_STR(num_workers <= runtime->num_workers || num_workers == 0, "Runtime system already initialized with %d workers which is lower than the requested %d workers.", runtime->num_workers, num_workers);
